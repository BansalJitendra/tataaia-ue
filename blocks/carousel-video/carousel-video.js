 ...
 * Horizontally scrolling video thumbnail cards with play overlay + caption.
...
function bindEvents(block) {
  ...
  block.querySelectorAll('.carousel-video-slide').forEach((slide) => slideObserver.observe(slide));
}
...
function createSlide(row, slideIndex, carouselId) {
  ...
  row.querySelectorAll(':scope > div').forEach((column, colIdx) => {
    column.classList.add(`carousel-video-slide-${colIdx === 0 ? 'image' : 'content'}`);
    slide.append(column);
  });
